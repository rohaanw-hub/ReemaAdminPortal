...
import {
  DAYS, ALL_TIME_SLOTS, WEEKDAY_SLOTS, SAT_SLOTS,
  isTutorAvailableAt, hasWeeklyConflict, timeToMinutes, SUBJECT_COLORS,
} from '../../helpers'

// ── Pure helpers ──────────────────────────────────────────────────────────────
function relScore(emp) {
  return emp.totalShifts === 0 ? 100 : Math.round((1 - emp.callouts / emp.totalShifts) * 100)
}
...
function findBestTutor(sessions, employees, weeklyConflicts, day, time, subject, excludeEmpId = null, excludeSessionId = null) {
  const candidates = employees.filter((emp) => {
    ...
    if (isDoubleBooked(sessions, emp.id, day, time, excludeSessionId)) return false
    return true
  })
...
// ── Conflicts Panel ───────────────────────────────────────────────────────────
function ConflictsPanel({ employees, weeklyConflicts, addWeeklyConflict, removeWeeklyConflict, clearWeeklyConflicts, onClose }) {
  const [selectedEmpId, setSelectedEmpId] = useState(employees[0]?.id ?? '')

  const blankForm = { day: 'Mon', startTime: '3PM', endTime: '4PM', reason: '' }
  const [form, setForm] = useState(blankForm)

  const setField = (k, v) => setForm((f) => ({ ...f, [k]: v }))

  const timeOptions = form.day === 'Sat' ? SAT_SLOTS : WEEKDAY_SLOTS
  const endOptions = timeOptions.filter((t) => timeToMinutes(t) > timeToMinutes(form.startTime))
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 520 }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          {empConflicts.length === 0 ? (
            <div className="text-sm" style={{ padding: '8px 0' }}>No conflicts set for this employee.</div>
          ) : (
            empConflicts.map((c) => (
              <div
                key={c.id}
              ...
              >
                <span style={{ fontWeight: 700, color: '#991b1b', fontSize: 12, minWidth: 32 }}>{c.day}</span>
                <span style={{ fontSize: 12, color: '#475569', minWidth: 80 }}>
                  {c.startTime === 'All Day' ? 'All Day' : `${c.startTime}–${c.endTime}`}
                </span>
                <span style={{ flex: 1, fontSize: 12, color: '#334155' }}>{c.reason}</span>
                <button
                  onClick={() => removeWeeklyConflict(Number(selectedEmpId), c.id)}
        ...
        <div style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 16, background: '#fafafa' }}>
          <div className="form-label" style={{ marginBottom: 12 }}>Add Conflict</div>
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setForm((f) => ({ ...f, day: d, startTime: opts[0], endTime: opts[1] || opts[0] }))
                }}
              >
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const eo = timeOptions.filter((t) => timeToMinutes(t) > timeToMinutes(st))
                  setForm((f) => ({ ...f, startTime: st, endTime: eo[0] || '' }))
                }}
              >
          ...
          {form.startTime !== 'All Day' && (
            <div className="form-group">
              ...
              >
                ...
                {endOptions.length === 0 && <option disabled>No valid end times</option>}
              </select>
            </div>
          )}
          ...
          <button className="btn btn-danger" onClick={handleAdd}>+ Add Conflict</button>
        </div>
...
function SessionChip({ session, students, employees, weeklyConflicts, onClick, isDragging, onDragStart, onDragEnd }) {
  ...
  const hasConflict =
    emp && !isCancelled && hasWeeklyConflict(weeklyConflicts, emp.id, session.day, session.time)
  ...
  return (
    ...
    >
      <div className="session-chip-name" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        {student?.name ?? '—'}
        {hasConflict && (
          <span title={`${emp.name} has a weekly conflict at this time`} style={{ color: '#d97706', fontSize: 12 }}>⚠</span>
        )}
      </div>
      <div className="session-chip-sub">
        {isCancelled ? 'Cancelled' : hasConflict ? `${emp.name.split(' ')[0]} — Conflict!` : emp ? emp.name.split(' ')[0] : 'Unassigned'}
        {' · '}{session.subject}
      </div>
    </div>
  )
...
// ── Week Grid ─────────────────────────────────────────────────────────────────
function WeekGrid({
  sessions, students, employees, weeklyConflicts,
  onClickSession,
  draggedSession, dragOverCell,
  checkDropValidity,
          ...
          {ALL_TIME_SLOTS.map((time) => (
            <tr key={time}>
              ...
              {DAYS.map((day) => {
                ...
                const cellSessions = sessions.filter((s) => s.day === day && s.time === time)

                // Check if any session in this cell has a tutor with a weekly conflict
                const cellHasConflict = !isUnavail && cellSessions.some((s) => {
                  const emp = employees.find((e) => e.id === s.employeeId)
                  return emp && s.status !== 'cancelled' && hasWeeklyConflict(weeklyConflicts, emp.id, day, time)
                })

                // Determine any employee who has a conflict in this slot (for the unavailability tint)
                const conflictedEmps = !isUnavail
                  ? employees.filter((emp) => hasWeeklyConflict(weeklyConflicts, emp.id, day, time))
                  : []
                ...
                if (isHovered && validity) {
                  cellStyle = validity.valid
                    ? { background: 'rgba(22,163,74,0.08)', outline: '2px dashed #16a34a', outlineOffset: '-2px' }
                    : { background: 'rgba(220,38,38,0.08)', outline: '2px dashed #dc2626', outlineOffset: '-2px' }
                } else if (cellHasConflict) {
                  cellStyle = { background: 'rgba(245,158,11,0.06)', outline: '1px solid #fde68a', outlineOffset: '-1px' }
                } else if (!isUnavail && conflictedEmps.length > 0 && cellSessions.length === 0) {
                  // Empty cell where a conflicted emp would normally work
                  cellStyle = { background: 'rgba(239,68,68,0.04)' }
                }

                const conflictTooltip = conflictedEmps.length > 0
                  ? conflictedEmps.map((e) => {
                      const c = (weeklyConflicts[e.id] || []).find((cf) => {
                        if (cf.day !== day) return false
                        if (cf.startTime === 'All Day') return true
                        const t = timeToMinutes(time)
                        return t >= timeToMinutes(cf.startTime) && t < timeToMinutes(cf.endTime)
                      })
                ...
                return (
                  ...
                  >
                    ...
                    {!isUnavail && cellSessions.length === 0 && conflictedEmps.length > 0 && !draggedSession && (
                      <div style={{ fontSize: 9, color: '#ef4444', textAlign: 'center', padding: '3px 0', opacity: 0.7 }}>
                        🚫 Unavailable
                      </div>
                    )}

                    {/* Drop hint for hovered empty cell */}
                    {isHovered && cellSessions.filter((s) => draggedSession?.id !== s.id).length === 0 && validity && (
                      <div
                        style={{
                          fontSize: 10,
                          textAlign: 'center',
...
function EmpSessionModal({ session, sessions, employees, students, weeklyConflicts, onClose, onUpdate }) {
  ...
  const availableEmps = employees.filter((emp) => {
    ...
    if (hasWeeklyConflict(weeklyConflicts, emp.id, session.day, session.time)) return false
    if (isDoubleBooked(sessions, emp.id, session.day, session.time, session.id)) return false
    return true
  })
  ...
  const handleReassign = () => {
    onUpdate({ ...session, employeeId: selectedEmpId ? Number(selectedEmpId) : null, status: 'scheduled' })
    onClose()
  }
  ...
  const handleFindReplacement = () => {
    const best = findBestTutor(sessions, employees, weeklyConflicts, session.day, session.time, session.subject, session.employeeId, session.id)
    if (best) {
    ...
    <div className="modal-overlay">
      <div className="modal modal-sm">
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div className="text-sm">{session.day} · {session.time} · {session.subject}</div>
          <div className="text-sm" style={{ marginTop: 4 }}>
            Tutor: {currentEmp?.name ?? 'Unassigned'}
          </div>
          {sessionHasConflict && (
            <div style={{ marginTop: 6, background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 6, padding: '6px 10px', fontSize: 12, color: '#92400e' }}>
              ⚠ {currentEmp.name} has a weekly conflict at this time.
            </div>
          )}
          {session.status === 'cancelled' && (
            <span className="badge badge-red" style={{ marginTop: 6 }}>Cancelled</span>
          )}
        </div>
        ...
        {session.status !== 'cancelled' && (
          <div className="form-group">
            ...
            {availableEmps.length === 0 && (
              <div className="text-sm" style={{ marginTop: 6 }}>No available tutors at this time.</div>
            )}
          </div>
        )}
        ...
        <div className="modal-footer" style={{ flexWrap: 'wrap' }}>
          {session.status === 'cancelled' ? (
            ...
            <>
              <button className="btn btn-danger btn-sm" onClick={handleCancel}>Mark Cancelled</button>
              {sessionHasConflict && (
                <button className="btn btn-warning btn-sm" onClick={handleFindReplacement}>Find Replacement</button>
              )}
              <button className="btn btn-outline btn-sm" onClick={onClose}>Close</button>
              <button className="btn btn-primary btn-sm" onClick={handleReassign}>Save Assignment</button>
            </>
          )}
        </div>
      </div>
...
function StuSessionModal({ session, sessions, employees, students, onClose, onUpdate, onMove }) {
  const student = students.find((s) => s.id === session.studentId)
  const emp = employees.find((e) => e.id === session.employeeId)
    ...
    <div className="modal-overlay">
      <div className="modal modal-sm">
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div className="text-sm" style={{ marginTop: 4 }}>{session.day} · {session.time} · {session.subject}</div>
          <div className="text-sm">Tutor: {emp?.name ?? 'Unassigned'}</div>
          {session.status === 'cancelled' && (
            <span className="badge badge-red" style={{ marginTop: 6, display: 'inline-flex' }}>Cancelled</span>
          )}
        </div>

        <div className="modal-footer" style={{ flexWrap: 'wrap' }}>
          {session.status !== 'cancelled' && (
            <button className="btn btn-danger btn-sm" onClick={handleCancel}>Cancel Session</button>
          )}
          <button className="btn btn-outline btn-sm" onClick={() => { onClose(); onMove(session) }}>
            Move Session
          </button>
          <button className="btn btn-outline btn-sm" onClick={onClose}>Close</button>
        </div>
...
// ── Move Session Modal ────────────────────────────────────────────────────────
function MoveModal({ session, sessions, employees, students, weeklyConflicts, onClose, onMove }) {
  const [newDay, setNewDay] = useState(session.day)
  const [newTime, setNewTime] = useState(session.time)
  const emp = employees.find((e) => e.id === session.employeeId)

  const tutorAvail = !emp || (isTutorAvailableAt(emp, newDay, newTime) && !hasWeeklyConflict(weeklyConflicts, emp.id, newDay, newTime))
  ...
  const handleMove = () => {
    ...
    if (!tutorAvail) {
      const best = findBestTutor(sessions, employees, weeklyConflicts, newDay, newTime, session.subject, null, session.id)
    ...
    <div className="modal-overlay">
      <div className="modal modal-sm">
        ...
        <div style={{ marginBottom: 16, fontSize: 13, color: '#475569' }}>
          Moving session for{' '}
          <strong>{students.find((s) => s.id === session.studentId)?.name ?? '—'}</strong>
        </div>
        ...
        {!tutorAvail && !conflict && (
          <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 7, padding: '8px 12px', fontSize: 12, color: '#92400e', marginBottom: 12 }}>
            Current tutor unavailable at this time. A replacement will be auto-assigned if possible.
          </div>
        )}
        {conflict && (
          <div style={{ background: '#fee2e2', border: '1px solid #fca5a5', borderRadius: 7, padding: '8px 12px', fontSize: 12, color: '#991b1b', marginBottom: 12 }}>
            Student already has a session at this time.
          </div>
        )}
        {tutorAvail && !conflict && (
          <div style={{ background: '#dcfce7', border: '1px solid #bbf7d0', borderRadius: 7, padding: '8px 12px', fontSize: 12, color: '#166534', marginBottom: 12 }}>
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" onClick={handleMove} disabled={conflict}>Confirm Move</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', gap: 12, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
      {Object.entries(SUBJECT_COLORS).map(([subj, { bg, color }]) => (
        <div key={subj} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11 }}>
        <div style={{ width: 10, height: 10, borderRadius: 3, background: '#f1f5f9', border: '1px solid #e2e8f0' }} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, marginLeft: 4, color: '#94a3b8' }}>
        <span style={{ color: '#16a34a', fontWeight: 600 }}>Green</span>=drop valid ·{' '}
        <span style={{ color: '#dc2626', fontWeight: 600 }}>Red</span>=conflict
      </div>
    </div>
  )
}
...
export default function Schedule() {
  ...
  const checkDropValidity = useCallback(
    (session, newDay, newTime) => {
      ...
      if (emp && !isTutorAvailableAt(emp, newDay, newTime)) {
        return { valid: false, reason: `${emp.name.split(' ')[0]} unavailable on ${newDay}` }
      }
      ...
      if (emp && hasWeeklyConflict(weeklyConflicts, emp.id, newDay, newTime)) {
        ...
        return { valid: false, reason: `${emp.name.split(' ')[0]}: ${conflictEntry?.reason ?? 'weekly conflict'}` }
      }

      if (emp && isDoubleBooked(sessions, emp.id, newDay, newTime, session.id)) {
        return { valid: false, reason: `${emp.name.split(' ')[0]} already booked at ${newTime}` }
      }
      ...
      if (activeTab === 'student') {
        ...
        if (hasConflict) {
          return { valid: false, reason: 'Student already scheduled at this time' }
        }
      }
  ...
  // DnD handlers
  const handleDragStart = useCallback((session) => setDraggedSession(session), [])
  const handleDragEnd = useCallback(() => { setDraggedSession(null); setDragOverCell(null) }, [])
  const handleCellDragEnter = useCallback((day, time) => setDragOverCell({ day, time }), [])
  ...
  const handleCellDrop = useCallback(
    (day, time) => {
      ...
      setSessions((prev) =>
        prev.map((s) =>
          s.id === draggedSession.id ? { ...s, day, time, status: 'scheduled' } : s,
        ),
      )
  ...
  const autoScheduleEmployees = () => {
    ...
    updated
      ...
      .forEach((session) => {
        const best = findBestTutor(updated, employees, weeklyConflicts, session.day, session.time, session.subject, null, session.id)
        if (best) {
      ...
      Object.entries(student.schedule).forEach(([day, times]) => {
        times.forEach((timeRange) => {
          ...
          for (const subject of student.subjects) {
            const emp = findBestTutor(updated, employees, weeklyConflicts, day, sessionTime, subject, null, null)
            if (emp) {
              updated.push({
                id: nextId++,
                day,
  ...
  const moveSession = (sessionId, newDay, newTime, newEmpId) => {
    setSessions((prev) =>
      prev.map((s) =>
        s.id === sessionId ? { ...s, day: newDay, time: newTime, employeeId: newEmpId, status: 'scheduled' } : s,
      ),
    )
    addNotification('info', `Session moved to ${newDay} ${newTime}.`)
  }

  const openMove = (session) => { setMoveTarget(session); setShowMoveModal(true) }

  const unassignedCount = sessions.filter((s) => !s.employeeId && s.status === 'scheduled').length
  const cancelledCount = sessions.filter((s) => s.status === 'cancelled').length
  const totalConflicts = Object.values(weeklyConflicts).reduce((sum, arr) => sum + (arr?.length ?? 0), 0)

  const dragProps = {
    draggedSession, dragOverCell, checkDropValidity,
    onDragStart: handleDragStart, onDragEnd: handleDragEnd,
    onCellDragEnter: handleCellDragEnter, onCellDrop: handleCellDrop,
  }
  ...
  return (
    <div>
      ...
      {activeTab === 'student' && (
        <div>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
            <button className="btn btn-primary" onClick={autoScheduleStudents}>Auto-Schedule Students</button>
            <span className="text-sm">Drag sessions to move · Click to cancel or move</span>
          </div>
          <Legend />
          <div className="card" style={{ padding: 12 }}>
            <WeekGrid
              sessions={sessions} students={students} employees={employees}
              weeklyConflicts={weeklyConflicts}
              onClickSession={(s) => { setSelectedSession(s); setShowStuModal(true) }}
              {...dragProps}
            />
          </div>
        </div>
      ...
      {showEmpModal && selectedSession && (
        <EmpSessionModal
          session={selectedSession} sessions={sessions} employees={employees}
      ...
      {showStuModal && selectedSession && (
        <StuSessionModal
          session={selectedSession} sessions={sessions} employees={employees}
          students={students}
          onClose={() => { setShowStuModal(false); setSelectedSession(null) }}
          onUpdate={updateSession}
          onMove={(s) => { setSelectedSession(null); openMove(s) }}
        />
      )}

      {showMoveModal && moveTarget && (
        <MoveModal
          session={moveTarget} sessions={sessions} employees={employees}
          students={students} weeklyConflicts={weeklyConflicts}
