...
import { getInitials, getAvatarBg, getAvatarText, calcHours } from '../../helpers'
...
export default function ClockIn() {
  ...
  const clockIn = (id) => {
    setEmployees((prev) =>
      prev.map((e) =>
        e.id === id
          ? { ...e, clockIns: [...e.clockIns, { in: new Date().toISOString(), out: null }] }
          : e
      )
    )
          ...
          <div key={emp.id} className="clock-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm">{emp.role} · {hours.toFixed(1)} hrs logged</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {isClockedIn ? (
                <button className="btn btn-danger" onClick={() => clockOut(emp.id)}>Clock Out</button>
              ) : (
                <button className="btn btn-success" onClick={() => clockIn(emp.id)}>Clock In</button>
              )}
            </div>
          </div>
        )
