...
import {
  getInitials, getAvatarBg, getAvatarText, formatDate,
  calcReliability, reliabilityColor, calcHours, DAYS, SUBJECTS, ED_LEVELS, ALL_TIME_SLOTS,
} from '../../helpers'
...
function EditModal({ emp, onClose, onSave }) {
  const initSchedule = () => {
    const s = {}
    DAYS.forEach((d) => {
      s[d] = { enabled: !!(emp.schedule[d]?.length), time: emp.schedule[d]?.[0] || '' }
    })
    return s
  }
    ...
    <div className="modal-overlay">
      <div className="modal">
        ...
        <div className="form-group" style={{ marginTop: 8 }}>
          ...
          <textarea className="form-textarea" value={form.conflicts} onChange={(e) => set('conflicts', e.target.value)} />
        </div>
        <div className="form-group">
...
export default function EmployeeProfile() {
  ...
  const blankConflict = { day: 'Mon', allDay: false, startTime: '3PM', endTime: '4PM', reason: '' }
  const [conflictForm, setConflictForm] = useState(blankConflict)

  const setConflict = (field, val) => setConflictForm((f) => ({ ...f, [field]: val }))
  ...
  const logCallout = () => {
    setEmployees((prev) =>
      prev.map((e) =>
        e.id === emp.id ? { ...e, callouts: e.callouts + 1, totalShifts: e.totalShifts + 1 } : e
      )
    )
  }
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 20, display: 'flex', alignItems: 'center', gap: 20 }}>
        <div
          className="avatar"
          style={{ width: 64, height: 64, fontSize: 22, background: getAvatarBg(emp.name), color: getAvatarText(emp.name) }}
        >
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 6, display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <span className="text-sm">{emp.callouts} callouts / {emp.totalShifts} shifts</span>
            <span className="text-sm">·</span>
      ...
      {tab === 'overview' && (
        <div className="profile-grid">
          <div className="card">
            ...
            <div className="detail-row"><div className="detail-label">Email</div><div className="detail-value">{emp.email}</div></div>
            <div className="detail-row"><div className="detail-label">Phone</div><div className="detail-value">{emp.phone}</div></div>
            <div className="detail-row"><div className="detail-label">Education</div><div className="detail-value">{emp.grade}</div></div>
            <div className="detail-row"><div className="detail-label">Subjects</div><div className="detail-value">{emp.subjects.join(', ')}</div></div>
            <div className="detail-row"><div className="detail-label">Hire Date</div><div className="detail-value">{formatDate(emp.hireDate)}</div></div>
            <div className="detail-row"><div className="detail-label">Hourly Rate</div><div className="detail-value">${emp.hourlyRate}/hr</div></div>
            {emp.conflicts && <div className="detail-row"><div className="detail-label">Conflicts</div><div className="detail-value">{emp.conflicts}</div></div>}
            {emp.notes && <div className="detail-row"><div className="detail-label">Notes</div><div className="detail-value">{emp.notes}</div></div>}
          </div>
          ...
          <div className="card">
            ...
            {empSessions.length === 0 ? (
              ...
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr><th>Day</th><th>Time</th><th>Student</th><th>Subject</th><th>Status</th></tr>
                  </thead>
                  <tbody>
                    {empSessions.map((s) => {
                      ...
                      return (
                        <tr key={s.id}>
                          ...
                          <td>{s.subject}</td>
                          <td>
                            <span className={`badge ${s.status === 'cancelled' ? 'badge-red' : 'badge-green'}`}>
                              {s.status}
                            </span>
                          </td>
                        </tr>
      ...
      {tab === 'schedule' && (
        <div className="card">
            ...
            {DAYS.map((day) => (
              <div key={day} className="day-col">
                ...
                {(emp.schedule[day] || []).map((slot) => (
                  <div key={slot} className="session-block">{slot}</div>
                ))}
                {!emp.schedule[day] && <div className="text-sm" style={{ textAlign: 'center' }}>—</div>}
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 24, borderTop: '1px solid #f1f5f9', paddingTop: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div className="section-title" style={{ marginBottom: 0 }}>
                ...
                {myConflicts.length > 0 && (
                  <span className="badge badge-red" style={{ marginLeft: 8 }}>{myConflicts.length}</span>
                )}
              </div>
              <button className="btn btn-outline btn-sm" onClick={() => setShowAddConflict((v) => !v)}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: myConflicts.length ? 12 : 0 }}>
              {myConflicts.map((c) => (
                <span
                  key={c.id}
                  style={{
                ...
                >
                  {c.day} {c.startTime === 'All Day' ? 'All Day' : `${c.startTime}–${c.endTime}`}
                  {c.reason ? ` — ${c.reason}` : ''}
                  <button
                    onClick={() => removeWeeklyConflict(emp.id, c.id)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#991b1b', fontWeight: 700, fontSize: 14, lineHeight: 1, padding: 0 }}
                  >×</button>
                </span>
              ))}
            </div>
            ...
            {showAddConflict && (
              <div style={{ background: '#fff8f8', border: '1px solid #fecaca', borderRadius: 8, padding: 14, marginTop: 8 }}>
                <div className="form-row" style={{ marginBottom: 10 }}>
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="form-label">Day</label>
                    <select className="form-select" value={conflictForm.day} onChange={(e) => setConflict('day', e.target.value)}>
                  ...
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6, fontSize: 13 }}>
                      <input type="checkbox" checked={conflictForm.allDay} onChange={(e) => setConflict('allDay', e.target.checked)} />
                      All Day
                    </label>
                  </div>
                </div>
                {!conflictForm.allDay && (
                  <div className="form-row" style={{ marginBottom: 10 }}>
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      ...
                      <select className="form-select" value={conflictForm.startTime} onChange={(e) => setConflict('startTime', e.target.value)}>
                        {ALL_TIME_SLOTS.map((t) => <option key={t}>{t}</option>)}
                      </select>
                    </div>
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      <label className="form-label">End Time</label>
                      <select className="form-select" value={conflictForm.endTime} onChange={(e) => setConflict('endTime', e.target.value)}>
                        {ALL_TIME_SLOTS.map((t) => <option key={t}>{t}</option>)}
                      </select>
                    </div>
                  </div>
                ...
                <button className="btn btn-danger btn-sm" onClick={handleAddConflict}>Add Conflict</button>
              </div>
            )}
          </div>
            ...
            {DAYS.map((day) => {
              ...
              return (
                <div key={day} style={{ marginBottom: 14 }}>
                  <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 6, color: '#475569' }}>{day}</div>
                  {daySessions.map((s) => {
                    const student = students.find((x) => x.id === s.studentId)
                    return (
                      <div key={s.id} style={{ display: 'flex', gap: 10, padding: '6px 0', borderBottom: '1px solid #f1f5f9', fontSize: 13 }}>
                        <span style={{ color: '#64748b', minWidth: 44 }}>{s.time}</span>
      ...
      {tab === 'hours' && (
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 20 }}>
            {[
              { label: 'Hours Logged', value: hours.toFixed(1), sub: 'this period' },
              { label: 'Total Shifts', value: emp.totalShifts, sub: 'all time' },
              { label: 'Est. Earnings', value: `$${(hours * emp.hourlyRate).toFixed(2)}`, sub: 'before deductions' },
            ].map((s) => (
          ...
          {emp.clockIns.length === 0 ? (
            ...
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>Clock In</th><th>Clock Out</th><th>Duration</th></tr>
                </thead>
                <tbody>
                  {emp.clockIns.map((c, i) => {
                    const dur = c.out ? ((new Date(c.out) - new Date(c.in)) / 3_600_000).toFixed(2) : '—'
                    return (
                      <tr key={i}>
                        <td>{new Date(c.in).toLocaleString()}</td>
                        <td>{c.out ? new Date(c.out).toLocaleString() : 'In progress'}</td>
                        <td>{dur !== '—' ? `${dur} hrs` : '—'}</td>
                      </tr>
                    )
                  })}
      ...
      {tab === 'reliability' && (
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
            {[
              { label: 'Reliability Score', value: `${rel}%`, color: reliabilityColor(rel) },
              { label: 'Callouts', value: emp.callouts, color: emp.callouts > 2 ? '#dc2626' : '#d97706' },
              { label: 'Total Shifts', value: emp.totalShifts, color: '#E31837' },
            ].map((s) => (
              <div key={s.label} className="stat-card">
                <div className="stat-label">{s.label}</div>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 13 }}>
              ...
              <span style={{ fontWeight: 700, color: reliabilityColor(rel) }}>{rel}%</span>
            </div>
            <div className="progress-bar" style={{ height: 10 }}>
              <div className="progress-fill" style={{ width: `${rel}%`, background: reliabilityColor(rel) }} />
            </div>
          </div>

          <div className="text-sm" style={{ marginTop: 12 }}>
            {rel >= 90 && 'Excellent reliability — fewer than 10% callout rate.'}
            {rel >= 75 && rel < 90 && 'Good reliability — minor improvement recommended.'}
